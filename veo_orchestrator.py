...
#         try:
#             # Preserve subfolder if present: /videos/job_xxx_date/filename.mp4
#             rel = video_url.lstrip("/")
#             if rel.startswith("videos/"):
#                 rel = rel[len("videos/"):]
#             local_path = self.stitcher.output_dir / rel

#             if not local_path.exists():
#                 logger.warning(f"   [FRAME_EXTRACT] Source clip not found: {local_path}")
#                 return None

#             # Write frame alongside the video in the same subfolder
...
#     # ── Short Span Clips ───────────────────────────────────────────────────────
#     # Each Excel row is one discrete clip prompt (no decomposition).
#     # Veo generates clips at the requested duration (2s is supported by API).
#     # Clips are chained via img2vid last-frame anchoring for visual continuity,
#     # then stitched into one output video.

#     async def generate_short_span_sequence(
#         self,
...
#     ) -> Dict[str, Any]:
#         """
#         Generate a sequence of short clips — one per Excel row — and stitch them.

#         Unlike the full-length pipeline, there is NO prompt decomposition here.
#         Each row = one clip sent directly to Veo. The user has full per-clip control.

#         Args:
#             prompts:         List of prompt dicts from excel_processor.
#             job_id:          Used in filenames.
#             clip_duration_s: Duration of each clip in seconds (2–8). Default 2s.
#                              Note: Veo supports any integer duration 2–8.
#             no_text:         Inject "no text overlay" guardrail into every prompt.
#             no_speech:       Inject "no speech/narration" guardrail into every prompt.
#         """
#         _NO_TEXT = (
#             "NO TEXT OVERLAY: Do not render any text, titles, captions, subtitles, "
#             "watermarks, or UI labels anywhere in the frame."
#         )
#         _NO_SPEECH = (
...
#         # Clip duration must be integer 2–8 for Veo API
#         clip_dur = max(2, min(8, int(clip_duration_s)))

#         logger.info(
#             f"[SHORT_SPAN] {job_id} — {n} clip(s), {clip_dur}s each, "
#             f"no_text={no_text}, no_speech={no_speech}"
#         )

#         for idx, prompt_data in enumerate(prompts):
#             raw_text = prompt_data.get("text", "")

#             # Inject guardrails at the front of every prompt
#             guardrails = []
#             if no_text:   guardrails.append(_NO_TEXT)
#             if no_speech: guardrails.append(_NO_SPEECH)
#             prompt_text = " ".join(guardrails + [raw_text]) if guardrails else raw_text

#             clip_label = f"ss_clip_{idx + 1:03d}"
#             logger.info(f"[SHORT_SPAN] Clip {idx + 1}/{n}: '{prompt_text[:70]}...'")
...
#                 logger.warning(f"[SHORT_SPAN] Clip {idx + 1} failed — skipping")
#                 failed += 1
#                 reference_image = None
#                 continue

#             clip_video_url = result["video_url"]
#             clip_local_path = Path(self._resolve_video_path(clip_video_url))
#             clip_paths.append(str(clip_local_path))

#             # Extract last frame for img2vid anchoring into next clip
#             frame = await self._extract_last_frame(
#                 video_url  = clip_video_url,
#                 clip_label = clip_label,
#             )
#             reference_image = frame
#             logger.info(f"[SHORT_SPAN] Clip {idx + 1} OK → {clip_video_url}")

#         if not clip_paths:
#             return {
#                 "status": "failed",
#                 "error_message": "All short-span clips failed to generate.",
#                 "clips_count": 0,
#             }
...
#         local_url = f"/videos/{Path(stitched_path).name}"
#         resolved  = self._resolve_video_path(local_url)
#         s3_url    = None
#         if self.s3 and self.s3.enabled:
#             s3_url = self.s3.upload_video(
#                 local_path   = resolved,
...
#             f"{len(clip_paths)} clips ok, {failed} failed"
#         )
...
#     def _resolve_video_path(self, video_url: str) -> str:
#         """
#         Convert a FastAPI route to an absolute filesystem path.

#         Handles both flat and subfolder structures:
#           /videos/filename.mp4                       → output_dir/filename.mp4
#           /videos/job_xxx_2026-04-03_13-41/file.mp4 → output_dir/job_xxx_.../file.mp4

#         The subfolder was introduced when per-job subfolders were added to
#         veo_generator.py. Path.name only gives the filename — we need the
#         full relative path after /videos/.
#         """
#         if not video_url:
#             return video_url
#         # Strip leading /videos/ and reconstruct under output_dir
#         rel = video_url.lstrip("/")
#         if rel.startswith("videos/"):
#             rel = rel[len("videos/"):]
...
#     def _resolve_local_paths(self, video_urls: List[str]) -> List[str]:
#         """Convert /videos/[subfolder/]filename URLs to absolute local paths for FFmpeg."""
#         local_paths = []
#         output_dir  = self.stitcher.output_dir

#         for url in video_urls:
#             # Strip /videos/ prefix and preserve subfolder if present
#             rel = url.lstrip("/")
#             if rel.startswith("videos/"):
#                 rel = rel[len("videos/"):]
