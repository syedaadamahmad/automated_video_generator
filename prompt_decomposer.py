...
# # ── Narrator voice-lock directive ─────────────────────────────────────────────
# _NARRATOR_DIRECTIVE_TPL = (
#     "NARRATOR VOICE: {narrator_desc}. "
#     "Maintain this exact voice, accent, tone, and pacing for the full duration of this clip. "
#     "Do not change narrator style mid-clip."
# )

# _NARRATOR_SYSTEM_ADDENDUM = """
# ═══ NARRATOR VOICE LOCK — MANDATORY RULE ════════════════════════════════════════
# The master prompt specifies a narrator voice. This must be consistent across all clips.
# - Every clip prompt MUST include the NARRATOR VOICE directive verbatim at the start.
# - The accent, tone, pacing, and gender of the narrator must NOT change between clips.
# - Narration lines must be distributed in order — do NOT reuse or skip any lines.
# - Each clip's narration must be completable within that clip's duration."""
...
# # ── System prompt template ────────────────────────────────────────────────────
# _SYSTEM_PROMPT = """You are an elite Hollywood trailer editor, cinematic commercial director, viral short-form strategist, and Veo AI video prompt architect — combined.

# Your task is to decompose a MASTER VIDEO PROMPT into exactly {n} sequential Veo-ready clip prompts that stitch into one seamless, emotionally powerful cinematic video.

# Think like Christopher Nolan, Zack Snyder, Denis Villeneuve, luxury ad directors, and viral reel editors combined.
# Every clip must feel: visually rich · emotionally escalating · cinematic · dynamic · immersive.
# NEVER generate flat storytelling. Every 2 seconds must escalate emotion OR reveal new information OR increase tension.

# ═══ STEP 1 — EXTRACT ANCHORS (lock these across ALL clips) ════════════════════
# Extract from the master prompt and hold constant across every clip:

# SCENE ANCHOR: Setting, lighting quality, color palette, time of day, atmosphere. 1–2 sentences maximum.
#   Bad: "A modern futuristic classroom filled with glowing AI holograms and machine learning neural network graphics..."
#   Good: "Futuristic classroom, glowing blue/purple AI holograms, neon-lit desks, large windows, daytime."

# CHARACTER ANCHOR (only if people present): Gender, approximate age, ethnicity, hair, ONE key clothing item per person. Max 20 words per character. No accessories, shoes, build, or posture.
#   Bad: "Student 1: Male, South Indian, dark brown neat side-part hair, crisp light blue button-down shirt, dark blue trousers, black leather shoes, slim build..."
#   Good: "Student 1: Indian male ~15, neat dark hair, light blue shirt. Student 2: Indian female ~15, black ponytail, turquoise top."

# CAMERA ANCHOR: Starting position, lens distance (close-up / medium / wide), angle. One sentence.

# ANCHOR BUDGET: SCENE + CHARACTER + CAMERA combined must be under 150 words. If over, cut character accessories first.

# ═══ STEP 2 — CINEMATIC WORLD AND CHARACTER DESIGN ════════════════════════════
# For each clip, write as a Hollywood production instruction. Every clip prompt MUST include:

# CINEMATOGRAPHY (vary across clips):
#   - Camera movement: dolly, push-in, pull-back, lateral tracking, static, aerial, whip pan, arc shot
#   - Lens type: wide angle, telephoto, macro, anamorphic
#   - Depth of field: shallow bokeh, deep focus, rack focus
#   - Speed: real-time, slow motion, speed ramp

# VISUAL PRODUCTION QUALITY (embed in every clip):
#   - Lighting: volumetric, golden hour, rim light, practical sources, HDR
#   - Texture: skin pores, fabric grain, environmental detail, particle effects
#   - Atmosphere: fog, dust, rain, ambient particles, god rays
#   - Color: rich cinematic palette from the SCENE ANCHOR

# EMOTIONAL STORYTELLING:
#   - Facial emotion (if characters present): specific expression, eyes, micro-expression
#   - Body language: deliberate and character-revealing
#   - Environmental storytelling: world reacts to the emotional moment

# ═══ STEP 3 — VISUAL HANDOFFS ════════════════════════════════════════════════
# For each clip N (except the last), define end_state: a precise single sentence describing the LAST VISIBLE FRAME.
# Clip N+1 must open from EXACTLY that visual state. This creates seamless stitching.

# ═══ STEP 4 — VIRAL RETENTION ENGINE ════════════════════════════════════════
# The clip sequence must follow this emotional escalation structure:
#   - Clips 1–2: Hook. First 3 seconds must stop the scroll. Visual surprise or bold statement.
#   - Clips 3–N-2: Escalation. Each clip increases tension, reveals new information, or deepens emotion.
#   - Clip N-1: Climax. Peak emotional or visual intensity.
#   - Clip N: Resolution. Payoff + close. Scene resolves naturally. Audio resolves.

# Pattern interrupts (use at least one per 3 clips):
#   - Unexpected camera angle
#   - Speed ramp transition
#   - Scale shift (extreme close-up to wide)
#   - Silence moment

# ═══ STEP 5 — WRITE EACH CLIP PROMPT ════════════════════════════════════════
# Rules (strictly enforced):
# 1. Embed SCENE ANCHOR and CHARACTER ANCHOR verbatim at the start of every prompt.
# 2. Clip action, cinematography, and narration come AFTER the anchors.
# 3. Vary ONLY: action progression, camera movement, emotional moment.
# 4. No new characters. No lighting changes. No setting changes. No costume changes.
# 5. Clip N+1 opens from clip N's end_state exactly.
# 6. Each prompt is fully self-contained — readable independently by a Veo model.
# 7. Write as Hollywood production instructions, not casual descriptions.

# ═══ NARRATION RULES (apply when master prompt contains "Narration" text) ═══
# A. COUNT then DISTRIBUTE. Count all narration lines. Divide: lines_per_clip = ceil(total_lines / {n}).
#    Assign exactly lines_per_clip consecutive lines per clip in order.
#    Do NOT drop any narration lines. Every line must appear in exactly one clip.
...
# C. FINAL CLIP closure. The last clip's prompt MUST signal that the video is ending:
#    - The final narration line must be a complete, resolving sentence.
#    - Add: "This is the final moment. The narration concludes naturally. Scene and audio resolve to a comfortable close."
#    - The visual action must reach a resting state (not mid-motion).

# D. NON-FINAL CLIPS: Each clip should feel like it could pause naturally — end on a beat, not mid-sentence.

# ═══ OUTPUT FORMAT ═══════════════════════════════════════════════════════════
# Return ONLY a JSON array of exactly {n} objects. No commentary. No markdown fences.
# Each object: {{"clip": <int>, "duration_s": <int>, "end_state": "<last frame>", "prompt": "<full self-contained Hollywood production prompt>"}}
...
# ── Gemini model IDs ──────────────────────────────────────────────────────────
MODEL_GEMINI_FLASH  = os.getenv("GEMINI_DECOMPOSER_MODEL", "gemini-2.5-flash")
...
class PromptDecomposer:
    ...
    def __init__(
        ...
        model_secondary: str = MODEL_DEEPSEEK,
        google_api_key: str = "",
        gemini_model: str = MODEL_GEMINI_FLASH,
    ):
        self.region          = region
        self.model_primary   = model_primary
        self.model_secondary = model_secondary
        ...
        try:
            ...
            logger.info("[DECOMPOSER] Bedrock client initialized (converse API)")
            if google_api_key:
                logger.info(f"   Primary:   Gemini ({gemini_model})")
                logger.info(f"   Secondary: {model_primary}")
                logger.info(f"   Tertiary:  {model_secondary}")
                logger.info(f"   Quaternary: deterministic phase templates")
            else:
                logger.info(f"   Primary:   {model_primary}")
                logger.info(f"   Secondary: {model_secondary}")
                logger.info(f"   Tertiary:  deterministic phase templates")
    ...
    def decompose(
        ...
        if self.google_api_key:
            result, source, clip_objects = self._try_gemini(
                master_prompt=master_prompt, n_clips=n_clips,
                clip_durations=clip_durations, platform=platform,
                is_static=is_static, narrator_desc=narrator_desc,
            )
            if result:
                return result, source, clip_objects, {
                    "input_tokens": total_input_tokens,
        ...
        if self._bedrock_client:
            ...
            if not self.google_api_key:
                logger.warning("[DECOMPOSER] No Gemini key, no Bedrock client — deterministic only")

        # ── Quaternary: deterministic ─────────────────────────────────────────
        used_deterministic = 1
        logger.warning("[DECOMPOSER] All LLM paths failed — using deterministic fallback")
        fallback_prompts, fallback_objects = self._phase_fallback(
            master_prompt, n_clips, clip_durations, is_static=is_static
    ...
    def _try_gemini(
        ...
        """Call Gemini 2.5 Flash via Google Generative Language REST API."""
        logger.info(f"[DECOMPOSER] Trying Gemini ({self.gemini_model})")
        try:
            result = self._llm_decompose_gemini(
                master_prompt=master_prompt, n_clips=n_clips,
                clip_durations=clip_durations, platform=platform,
                is_static=is_static, narrator_desc=narrator_desc,
            )
            ...
            sub_prompts, clip_objects = result
            if sub_prompts and len(sub_prompts) == n_clips:
                logger.info(f"[DECOMPOSER] Gemini succeeded ({n_clips} clips)")
                return sub_prompts, "Gemini", clip_objects or []
            count = len(sub_prompts) if sub_prompts else 0
            logger.warning(f"[DECOMPOSER] Gemini returned {count}/{n_clips} clips")
            return None, "Gemini", []
        except Exception as e:
            logger.error(f"[DECOMPOSER] Gemini failed: {e}")
            return None, "Gemini", []

    def _llm_decompose_gemini(
        self,
        master_prompt: str, n_clips: int,
        clip_durations: List[int], platform: str,
        is_static: bool = False,
        ...
        """Call Gemini REST API — same prompt, same JSON parsing as Bedrock path."""
        import os as _os
        total_duration    = sum(clip_durations)
        clip_duration_str = " + ".join(f"{d}s" for d in clip_durations)
        ...
        raw_text = self._gemini_converse(user_content)
        logger.info(f"[DECOMPOSER] Gemini raw: '{raw_text[:200]}'")

        clip_objects = self._parse_json_objects(raw_text, n_clips)
        if clip_objects:
            sub_prompts = [obj["prompt"] for obj in clip_objects]
            if len(sub_prompts) != n_clips:
                sub_prompts, clip_objects = self._repair_count(
                    sub_prompts, n_clips, master_prompt, clip_objects, clip_durations
                )
            sub_prompts, clip_objects = self._inject_static(sub_prompts, clip_objects, is_static)
            if narrator_desc:
                sub_prompts, clip_objects = self._inject_narrator(sub_prompts, clip_objects, narrator_desc)
        ...
        if len(sub_prompts) != n_clips:
            sub_prompts, clip_objects = self._repair_count(
                sub_prompts, n_clips, master_prompt, [], clip_durations
            )
        else:
            clip_objects = [
                {"clip": i + 1, "duration_s": clip_durations[i], "prompt": sp, "end_state": ""}
    ...
    def _gemini_converse(self, user_text: str) -> str:
        """
        Call Google Generative Language API (same key as Veo and Imagen).
        No new credentials needed — uses self.google_api_key.
        """
        url = _GEMINI_API_URL.format(model=self.gemini_model)
        payload = {
        ...
        try:
            ...
            raise RuntimeError(f"Gemini unexpected response shape: {e} — {data}")
